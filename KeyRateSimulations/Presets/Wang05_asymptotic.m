%Preset input for prepare-and-measure Wang05 protocol (decoherence only)
% 2 photon protocol
% the two angles for the noise are currently fixed parameters but can also
% be scan parameters for a sweep

function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = Wang05_asymptotic()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    protocolDescription=str2func('Wang05LossyDescription');
    channelModel=str2func('Wang05LossyNoisyChannel');
    %protocolDescription=str2func('Wang05_rot_LossyDescription');
    %channelModel=str2func('Wang05_rot_LossyNoisyChannel');
...
function parameters=setParameters()
    ...
    %parameters.names = ["loss", "f","fullstat", "num_pts", "phi"];
    ...
    parameters.scan.loss = 10*(0:5:350)/100; % total loss
    %parameters.scan.theta = (0:0.01:0.5)*pi;
    %parameters.scan.phi = (0:0.004:0.5)*pi;
    %parameters.scan.num_pts = (0:10:100);
    ...
    parameters.fixed.theta = 0.0*pi; % misalignment angle for channel unitary
    parameters.fixed.phi = 0.0*pi; % phase misalignment angle for channel unitary
