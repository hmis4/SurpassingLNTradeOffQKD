...
%% FUNCTION NAME: Boileau4LossyDescription
% Lossy description for prepare-and-measure Boileau protocol, 4 photons, decoherence free
%%

function protocolDescription = Boileau4LossyDescription(names,p)
    ...
    % previous version
    % bobConclusivePOVM = 0.5*(Pi_bx + Pi_bz + Pi_cx + Pi_cz); % Hannah version, pinched projectors
    % sqrtBobconclusivePovm = sqrtm(bobConclusivePOVM);
    % krausOpC = kron(kron(kron(zket(2,1),diag([1,0,0,0])) ...
    %     + kron(zket(2,2),diag([0,1,0,0])), ...
    %     blkdiag(sqrtBobconclusivePovm,[0])), [1]);  % conclusive
    % krausOp = {krausOpC};
    
    % Boileau protocol, keymap on Bob. 
    % in boileau there is no Pi_az or Pi_ax so possibly can save some dimensions
    % z basis alice, zbasis bob
    krausOpCZZ = kron(kron(kron(zket(2,2),diag([1,1,0,0])), blkdiag(Pi_bz/sqrt(2),[0])), zket(4,1))+...
        kron(kron(kron(zket(2,1),diag([1,1,0,0])), blkdiag(Pi_cz/sqrt(2),[0])), zket(4,1));
    
    % x basis alice, zbasis bob
    krausOpCXZ = kron(kron(kron(zket(2,2),diag([0,0,1,1])), blkdiag(Pi_cz/sqrt(2),[0])), zket(4,3))+...
        kron(kron(kron(zket(2,1),diag([0,0,1,1])), blkdiag(Pi_dz/sqrt(2),[0])), zket(4,3));
    
    % z basis alice, xbasis bob
    krausOpCZX = kron(kron(kron(zket(2,2),diag([1,1,0,0])), blkdiag(Pi_bx/sqrt(2),[0])), zket(4,2))+...
        kron(kron(kron(zket(2,1),diag([1,1,0,0])), blkdiag(Pi_cx/sqrt(2),[0])), zket(4,2));

    % x basis alice, xbasis bob
    krausOpCXX = kron(kron(kron(zket(2,2),diag([0,0,1,1])), blkdiag(Pi_cx/sqrt(2),[0])), zket(4,4))+...
        kron(kron(kron(zket(2,1),diag([0,0,1,1])), blkdiag(Pi_dx/sqrt(2),[0])), zket(4,4));
    ...
    % basicBobPOVMs = {0.5*(blkdiag(Pi_bz, [0])),...
    %     0.5*(blkdiag(Pi_cz, [0])), ...
    %     0.5*(blkdiag(Pi_dz, [0])), ...
    %     0.5*(blkdiag(Pi_bx, [0])), ...
    %     0.5*(blkdiag(Pi_cx, [0])), ...
    %     0.5*(blkdiag(Pi_dx, [0])), ...
    %     blkdiag(eye(16) - 0.5*(Pi_bz + Pi_cz + Pi_dz + Pi_bx + Pi_cx + Pi_dx), [0]), ...
    %     blkdiag(zeros(16), [1]), 
    %     };  % only conclusive onces
