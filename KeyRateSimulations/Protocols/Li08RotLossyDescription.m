...
function protocolDescription = Li08RotLossyDescription(names,p)
    ...
    dimPB = 10; % a,b,c,d in each basis, rest and noclick
    ...
    % Li rotation protocol. create Kraus operator after keymap on Bob
    % z basis alice, zbasis bob
    krausOpCZZ = kron(kron(kron(zket(2,1),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_az,[0])), zket(4,1)) + ...
        kron(kron(kron(zket(2,2),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_dz,[0])), zket(4,1))+...
        kron(kron(kron(zket(2,2),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_cz,[0])), zket(4,1))+...
        kron(kron(kron(zket(2,1),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_bz,[0])), zket(4,1));
    % TODO missing one outcome ??????
    
    % x basis alice, zbasis bob
    krausOpCXZ = kron(kron(kron(zket(2,1),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_az,[0])), zket(4,3)) + ...
        kron(kron(kron(zket(2,2),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_dz,[0])), zket(4,3))+...
        kron(kron(kron(zket(2,2),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_bz,[0])), zket(4,3))+...
        kron(kron(kron(zket(2,1),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_cz,[0])), zket(4,3));    
    
    % z basis alice, xbasis bob
    krausOpCZX = kron(kron(kron(zket(2,1),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_ax,[0])), zket(4,2)) + ...
        kron(kron(kron(zket(2,2),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_dx,[0])), zket(4,2))+...
        kron(kron(kron(zket(2,2),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_cx,[0])), zket(4,2))+...
        kron(kron(kron(zket(2,1),diag([1,1,0,0])), blkdiag(sqrt(0.5)*Pi_bx,[0])), zket(4,2));

    % x basis alice, xbasis bob
    krausOpCXX = kron(kron(kron(zket(2,1),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_ax,[0])), zket(4,4)) + ...
        kron(kron(kron(zket(2,2),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_dx,[0])), zket(4,4))+...
        kron(kron(kron(zket(2,2),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_bx,[0])), zket(4,4))+...
        kron(kron(kron(zket(2,1),diag([0,0,1,1])), blkdiag(sqrt(0.5)*Pi_cx,[0])), zket(4,4));
